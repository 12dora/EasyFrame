...
import { dismissAllNotifications, dismissNotification, loadGeneralSettings, loadNotifications, loadShellIdentity, type ShellGeneralSettings, type ShellIdentity } from "../lib/shell-adapter";
...
export function BlankShell({ children, locale: rawLocale }: { children: ReactNode; locale: string }) {
  ...
  useEffect(() => { let active = true; const update = (event: Event) => setSettings((event as CustomEvent<ShellGeneralSettings>).detail); loadGeneralSettings().then((value) => { if (active) setSettings(value); }).catch(() => { if (active) setSettings(null); }); window.addEventListener("enterprise-starter:general-updated", update); return () => { active = false; window.removeEventListener("enterprise-starter:general-updated", update); }; }, []);
  ...
  const topbar = <Topbar brand={
    ...
    <Link href={href("/app")} className="min-w-0"><div className="truncate text-[16px] font-semibold text-ink">{t.brand}</div><p className="hidden text-[12px] text-ink-faint sm:block">{t.subtitle}</p></Link>
