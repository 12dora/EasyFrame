...
import { AppShell, MobileNav, Sidebar, Topbar, type NavModel, type NavPanel, type RenderNavLink } from "@easy-enterprise/ui/shell";
import { PageLoadingSkeleton } from "@easy-enterprise/ui";
import { EnterpriseConfiguredFooter, EnterprisePublicShell, EnterpriseTopbarActions, performEnterpriseLogout, type EnterpriseNotification } from "@easy-enterprise/ui/enterprise";
...
import { dismissAllNotifications, dismissNotification, loadFooterSettings, loadNotifications, loadShellIdentity, type ShellFooterSettings, type ShellIdentity } from "../lib/shell-adapter";
...
export function BlankShell({ children, locale: rawLocale }: { children: ReactNode; locale: string }) {
  ...
  useEffect(() => { let alive = true; const forcedTarget = `/${locale}/app/settings/security/password`; loadShellIdentity(t.brand, t.access.authorization.roleGroupSeparator, t.common.notAvailable).then((value) => { if (!alive) return; if (value.mustChangePassword && pathname !== forcedTarget) { router.replace(forcedTarget); return; } setIdentity(value); }).catch(() => { if (alive) { logout(); router.replace(`/${locale}/login?next=${encodeURIComponent(pathname)}`); } }); return () => { alive = false; }; }, [locale, pathname, router, t]);
  useEffect(() => {
    const invalidate = () => {
      setIdentity(null);
  ...
  useEffect(() => { let active = true; const update = (event: Event) => setFooter((event as CustomEvent<ShellFooterSettings>).detail); loadFooterSettings().then((value) => { if (active) setFooter(value); }).catch(() => { if (active) setFooter(null); }); window.addEventListener("enterprise-starter:footer-updated", update); return () => { active = false; window.removeEventListener("enterprise-starter:footer-updated", update); }; }, []);
  ...
  if (identity.mustChangePassword && pathname === forcedTarget) return <BlankShellIdentityContext.Provider value={identity}><EnterprisePublicShell topbar={topbar} footer={<EnterpriseConfiguredFooter html={footerHtml} fallback={<>{t.public.footer} · © {new Date().getFullYear()}</>}/>}>{children}</EnterprisePublicShell></BlankShellIdentityContext.Provider>;
  const openPanelId = pathname.includes("/settings/") ? panel : null;
  return <BlankShellIdentityContext.Provider value={identity}><MotionConfig reducedMotion="user"><AppShell topbar={topbar} sidebar={<Sidebar model={model} openPanelId={openPanelId} onOpenPanel={openPanel} onBack={() => setPanel(null)} renderLink={renderLink} backLabel={t.navigation.backToMain} navLabel={t.navigation.menu}/>} mobileNav={<MobileNav model={model} renderLink={renderLink} backLabel={t.navigation.backToMain} menuLabel={t.navigation.menu} closeLabel={t.navigation.close} navLabel={t.navigation.menu} pathKey={pathname}/>} mainClassName="pb-12">{children}</AppShell></MotionConfig></BlankShellIdentityContext.Provider>;
}
