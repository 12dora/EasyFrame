...
```json
{
  ...
  "preferences": { "tableDensity": "compact", "rowSpacing": "compact" }
}
```
  ...
  - `preferences.tableDensity` 与 `preferences.rowSpacing` 分别为 `"compact"` | `"comfortable"`，来自账号 `platform_accounts.ui_preferences` 的 `table_density` 与 `row_spacing`；键缺失或非法值时全局缺省 `"compact"`。
- `PATCH /auth/preferences` 请求体为 `{ "tableDensity"?: "compact" | "comfortable", "rowSpacing"?: "compact" | "comfortable" }`（`StrictPlatformModel`，只接受 camelCase）。省略或 `null` 的字段不改原值，未知键或非法值 422。成功返回更新后的 `AuthSession`。变更写入 `PlatformAuditLog`（`auth.preferences.update`）。
- 不带回接入状态或密钥。
...
- `/auth/session` 与 `/auth/preferences` 挂在路由组 `me`。`me=False` 的宿主（EasyTrade web 自建 `/auth/me`）拿不到共享路由，必须自己提供 `permissionRequestUrl` 与两项 `preferences`。
- **平台表必须与 blank 一致**：宿主复制 blank 的 alembic 迁移 `0006_ui_preferences`，给 `platform_accounts` 增加 `ui_preferences`（JSON，`NOT NULL`，server default `{}`）；账户模型加同名列；AccountPort 实现 `get_ui_preferences` / `update_ui_preferences`。JSON 内键为 `table_density` 与 `row_spacing`（均为 `compact` | `comfortable`，独立默认紧凑）；只新增 JSON 键不需要新迁移。
