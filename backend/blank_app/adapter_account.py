...
def _persist_passkey_challenge(db, *, account_id: str, purpose: str, issued) -> None:
    db.execute(
        delete(_facade().PlatformSetting).where(
            _facade().PlatformSetting.key.like(f"{_PASSKEY_CHALLENGE_KEY_PREFIX}%"),
            _facade().PlatformSetting.value["expires_at"].as_integer() < int(datetime.now(UTC).timestamp()),
        )
    )
    db.add(
...
def _consume_passkey_challenge(db, *, account_id: str, purpose: str, jti: str, now: datetime) -> bool:
    ...
    result = db.execute(
        ...
        .where(
            _facade().PlatformSetting.key == _passkey_challenge_key(jti),
...
class BlankLocalAccountUnitOfWork:
    ...
    def list_accounts(self, *, search: str | None) -> list[LocalAccountRecord]:
        passkey_counts = (
            self.db.query(_facade().Passkey.account_id, func.count(_facade().Passkey.id))
            .group_by(_facade().Passkey.account_id)
            .subquery()
        )
        query = (
            self.db.query(_facade().Account, func.coalesce(passkey_counts.c.count, 0))
        ...
        if search and search.strip():
            term = f"%{search.strip()}%"
            query = query.filter(or_(_facade().Account.username.ilike(term), _facade().Account.email.ilike(term)))
    ...
    def get_account(self, account_id: uuid.UUID, *, for_update: bool = False) -> LocalAccountRecord | None:
        ...
        if account is None:
            return None
        passkey_count = int(
            self.db.query(func.count(_facade().Passkey.id))
    ...
    def usable_local_admin_count(self, *, now: datetime) -> int:
        self.db.flush()
        return int(
            self.db.query(func.count(_facade().Account.id))
            .filter(
                _facade().Account.external_source.is_(None),
                _facade().Account.is_admin.is_(True),
                _facade().Account.active.is_(True),
                or_(_facade().Account.expires_at.is_(None), _facade().Account.expires_at > now),
            )
            .scalar()
            or 0
        )
    ...
    def create_account(self, **values: Any) -> LocalAccountRecord:
        ...
        account = _facade().Account(password_hash=pwd_context.hash(password), **values)
        self.db.add(account)
        try:
            self.db.flush()
        except IntegrityError as exc:
